...
class FileSecurityValidator:
	...
	def __init__(self):
		...
		self.allowed_mime_types = {"application/pdf": [".pdf"], "application/vnd.openxmlformats-officedocument.wordprocessingml.document": [".docx"]}
	...
	def _detect_mime_type(self, file_content: bytes) -> Optional[str]:
		...
		try:
			# Use python-magic if available, otherwise fallback to basic detection
			try:
				import magic
				...
				# Fallback to basic magic number detection
				return self._basic_mime_detection(file_content)
		except Exception as e:
