#!/usr/bin/env python3
"""
Seed ChromaDB with sample precedent clauses for contract analysis.

This script populates the vector store with realistic legal precedent clauses
that can be used for RAG-based contract analysis.
"""
...
def get_sample_precedents() -> list[PrecedentClause]:
...
async def seed_vector_store():
	"""Seed the vector store with sample precedent clauses."""
	try:
		logger.info("Starting to seed ChromaDB with precedent clauses...")
		...
		# Add precedents to the vector store
		vector_store.add_precedent_clauses(precedents)

		# Get collection stats to verify
		stats = vector_store.get_collection_stats()

		logger.info(f"Successfully seeded ChromaDB with {len(precedents)} precedent clauses")
...
async def main():
	...
	logger.info("ChromaDB Precedent Seeding Script")
	logger.info("=" * 40)
